...
class Dashboard extends CI_Controller {
	...
	{
		$data['tags'] = $this->m_data->get_data('maps')->result();
		$this->load->view('dashboard/v_header');
		$this->load->view('dashboard/v_maps',$data);
		$this->load->view('dashboard/v_footer');
	}

	public function maps_tambah()
	{
		$this->load->view('dashboard/v_header');
		$this->load->view('dashboard/v_tags_tambah');
		$this->load->view('dashboard/v_footer');
	}

	public function maps_aksi()
	{
		$this->form_validation->set_rules('maps','maps','required');

		if($this->form_validation->run() != false){

			$tags = $this->input->post('maps');

			$data = array(
				'maps_nama' => $tags,
				'maps_slug' => strtolower(url_title($tags))
			);

			$this->m_data->insert_data($data,'tags');

			redirect(base_url().'dashboard/tags');
	...
	public function tags_edit($id)
	{
		...
		$data['tags'] = $this->m_data->edit_data($where,'tags')->result();
		$this->load->view('dashboard/v_header');
		$this->load->view('dashboard/v_tags_edit',$data);
		$this->load->view('dashboard/v_footer');
	}
	...
	public function tags_update()
	{
		...
		if($this->form_validation->run() != false){
			...
			$data = array(
				'tags_nama' => $tags,
				'tags_slug' => strtolower(url_title($tags))
			);

			$this->m_data->update_data($where, $data,'tags');
	...
	public function tags_hapus($id)
	{
		$where = array(
			'tags_id' => $id
		);

		$this->m_data->delete_data($where,'tags');
